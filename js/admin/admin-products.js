    ...
    async function initializeProducts() {
      try {
        ...
        if (result === 200) {
          ...
          console.warn('Failed to fetch products, status:', result);
    ...
    document.getElementById('product-form').addEventListener('submit', async (e) => {
      ...
      try {
        if (productId) {
          ...
          const responsePhoto = await uploadProductPhoto(accessToken,file,idProduct)
          console.log('registerProduct response:', response);
          if (response === 200 && responsePhoto.status === 200) {
