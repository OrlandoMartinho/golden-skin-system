...
const openModalInscricao = async () => {
    ...
    try {
        ...
        const response = await fetch("../../components/usuarios/editar-inscricao.html");
        if (!response.ok) {
...
const openModalDetalhes = async () => {
    

    try {
        // Carregar o modal dinamicamente
        const response = await fetch("../../components/usuarios/detalhes-modal.html");
        if (!response.ok) {
            throw new Error(`Erro ao carregar o modal: ${response.statusText}`);
        }
        const html = await response.text();
        ...
        configurarModalDetalhes(); // Inicializa os eventos do modal
    } catch (error) {
        console.error("Erro ao carregar o modal:", error);
    }
};
...
function closeModal() {
    document.getElementById("detalhes-modal").close();
}